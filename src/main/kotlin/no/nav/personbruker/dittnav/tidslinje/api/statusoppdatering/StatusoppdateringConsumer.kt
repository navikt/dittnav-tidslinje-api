...
import no.nav.personbruker.dittnav.tidslinje.api.config.getWithParams
import java.net.URL

class StatusoppdateringConsumer(
        private val client: HttpClient,
        private val eventHandlerBaseURL: URL,
        private val pathToEndpoint: URL = URL("$eventHandlerBaseURL/fetch/statusoppdatering/all")
) {
    ...
    private suspend fun getExternalEvents(innloggetBruker: InnloggetBruker,
                                          ...
                                          grupperingsId: String,
                                          produsent: String): List<Statusoppdatering> {
        return client.getWithParams(completePathToEndpoint, innloggetBruker, grupperingsId, produsent)
    }
}
