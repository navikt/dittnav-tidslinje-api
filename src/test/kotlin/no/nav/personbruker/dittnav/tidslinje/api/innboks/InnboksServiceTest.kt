...
class InnboksServiceTest {
    ...
    fun `should return list of InnboksDTO when Events are received`() {
        ...
        coEvery { innboksConsumer.getExternalEvents(innloggetBruker, grupperingsid, systembruker) } returns listOf(innboks1, innboks2)
        runBlocking {
            val innboksList = innboksService.getInnboksEvents(innloggetBruker, grupperingsid, systembruker)
    ...
    fun `should throw exception if fetching events fail`() {
        coEvery { innboksConsumer.getExternalEvents(innloggetBruker, grupperingsid, systembruker) } throws Exception("error")
        invoking {
            runBlocking {
                innboksService.getInnboksEvents(innloggetBruker, grupperingsid, systembruker)
            }
